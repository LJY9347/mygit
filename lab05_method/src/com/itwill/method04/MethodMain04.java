...
public class MethodMain04 {
	...
	/**
	 * 아규먼트로 전달 정수가 짝수인지, 홀수인지를 리턴하는 메서드.
	 * 
	 * @param a 정수(int). 짝수/홀수를 검사할 정수.
	 * @return a가 짝수이면 true, 그렇지 않으면 false.
	 */
	public static boolean isEven(int a) {// parameter는 반환값과 다른 타입이여도 가능.
		if (a % 2 == 0) {
			return true; // 메서드 안에서 리턴문은 항상 한번만 실행됨.
		} else {
			return false;
		}
	}
//			return (a % 2 == 0 ) ? true : false;
	 ...
	 * @param number 성별 코드. 1, 2, 3, 4.
	 * @return 넘버 값이 1또는 3이면 "male", 코드 값이 2또는 4이면 "female".
	 * 만약 그 이외의 코드 값인 경우에는 "unknown"을 리턴.
	 */
	
	public static String parseGenderCode(int number) {
