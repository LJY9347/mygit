...
public class ArrayMain02 {
	...
	public static void main(String[] args) {
		...
		// 타입[] 변수이름 = new 타입[] {...};  -> 어려운 방법. (주의) 배열 원소 개수를 표기하지 않음.
		// 타입[] 변수이름 = {...};
		int[] numbers = {1, 3, 5, 7}; //
		System.out.println(numbers[0]);//numbers[i] 인덱스 몇번지를 꺼내오겠다.
		for (int i = 0; i < numbers.length; i++) {
			System.out.println(numbers[i]); //numbers[i] 인덱스 몇번지를 꺼내오겠다.
		}
		
		// 향상된 for 문 (enhanced for statement, for-each) 
		// for ( 변수 선언 : 배열이름 ) {...}
		// 배열의 원소들을 순서대로 반복(iteration)하면서 반복문 블록을 수행 
		// 배열 원소의 값을 읽을 수는 있지만, 변경할 수는 없다 -> 값을 순서대로 읽을때만 편리.
		for (int x : numbers) {
		...
		// 문자열 3개를 저장할 수 있는 배열을 선언, 기본값으로 초기화.
