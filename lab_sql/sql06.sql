 ...
 */
 ...
 -- left outer join에서 outer은 생략 가능.
...
-----
 select e1.*,e2.*,s.*,e2.sal
 from emp e1
    join emp e2 on e1.mgr = e2.empno
    join salgrade s on e1.sal between s.losal and s.hisal;


 select e.ename, e.sal, s.grade, e1.ename as MGR_NAME
 from emp e
    join salgrade s on e.sal between s.losal and s.hisal
    join emp e1 on e.mgr = e1.empno
    order by e1.ename,e.sal;
...
--      부서의 급여 최댓값 검색. 부서 번호 오름 차순.
 select d.deptno, d.dname, min(e.sal),max(e.sal),count(*)
 from emp e
    join dept d on e.deptno = d.deptno
...
--      정렬 순서: (1) 부서 번호, (2) 사번 오름차순.
 ...
 select d.deptno, d.dname, e1.empno, e1.ename ,e2.empno, e2.ename,e1.sal
 , s.grade
 from emp e1, emp e2, dept d, salgrade s
 where e1.mgr = e2.empno and e1.deptno = d.deptno and 
 e1.sal >= 3000 and e1.sal between losal and hisal
