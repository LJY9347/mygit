...
where e.department_id = d.department_id and to_char(hire_date,'yyyy') = '2008'
...
(select round(max(avg(salary)),1)from employees group by department_id);
-- having round(avg(salary),1) =  급여의 평균이 최댓값과 같아야 한다.
-- 부서별 평균에서 최댓값을 찾겠다.
...
select j.job_title, round(avg(e.salary),1),min(e.salary),max(e.salary)
from employees e ,jobs j
where e.job_id = j.job_id
