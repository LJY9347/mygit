 ...
 */
 ...
 --> SQL에서 null 여부 비교를 할때는 반드시 is null, is not null을 사용!
 ...
 --> SQL에서 문자열은 작은따옴표('')을 사용. 대/소문자를 구분.
 ...
 -- 숫자 타입 뿐만 아니라, 문자열과 날짜 타입도 대/소 비교가 가능.
 ...
 select * from emp
 where hiredate > to_date('1987/01/01') --> 명시적 타입 변환:
 order by hiredate;
 -- to date('문자열'): '문자열'을 날짜(date) 타입으로 변환.
...
select * from emp
where ename like 'A____'; -- 글자 갯수가 언더스코어 갯수만큼 있어야 함.
