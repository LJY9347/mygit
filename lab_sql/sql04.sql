 ...
 *    행 마다 하나씩 결과가 리턴되는 함수. 행 갯수만큼 결과가 나옴.
...
-- 변수가 null이면 값2를 리턴.
select comm, nvl(comm, -1), nvl2(comm, comm, 0), nvl2(comm,'true','false')
from emp;
...
select lower('Allen') from dual; 
-- dual은 select / from 문법을 맞추기 위한 더미데이터
...
-- 사번, 이름, 급여, 수당, 연봉을 검색.
-- 연봉 = 급여 * 12 + 수당.
select empno, ename, sal + comm as "숫자로 이루어진 
컬럼끼리는 서로 더해짐", sal * 12 + nvl(comm,0) as "연봉"
from emp;
...
-- 데이터베이스의 테이블은 직사각형 모양.
--select sal, sum(sal) from emp; 불가능 출력되는 행의 개수가 다름!
...
select job, count(*), max(sal), min(sal), round(avg(sal),2)
from emp
group by job
order by job;
...
select deptno, to_char(hiredate,'yyyy') as YEAR, count(*)
from emp
