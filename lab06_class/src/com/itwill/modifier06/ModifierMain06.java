...
/*
 * 접근 수식어(접근 제한자 access modifier)는 지역 변수에는 사용할 수 없음!
 * 
 * final 수식어
 ...
 * 	(2) 객체를 생성하지 않아도 사용할 수 있는 멤버.(↓:클래스이름.↓필드							)
 ...
 *  (6) static(정적) 메서드는 static 으로 선언된 필드와 메서드만 사용 가능.(예) 29번줄에서 math. 해보기
 */
...
//		app.message = ""; //-> final 필드는 객체 생성 이후에 값을 변경할 수 없다.

		// Test 클래스의 정적(static) 멤버 사용:
		System.out.println("Test.y = " + Test.y);
		Test.y = 100; // static 필드는 객체 생성과 관계없이 사용할 수 있다.
		System.out.println("Test.y = " + Test.y);
		Test.printFields2();
		
		// Test 클래스의 인스턴스(static이 아닌) 멤버 사용: 먼저 객체를 생성해야 한다.
		Test test = new Test();
