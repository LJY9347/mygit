...
public class Score {
	// 필드(field). 멤버 변수. class안에서 선언되는 변수.
	int java;
	int sql;
	int javascript;
	...
	public Score(int java, int sql, int js) { 
		this.java = java;
		this.sql = sql;
		javascript = js; 		//필드와 파라미터의 이름이 다르다면 this.생략가능
	}
	
	// 메서드(method).
	public int getTotal() { // 클래스가 가지고있는 필드들로 필요한 기능을 만듦.
		return java + sql + javascript; // 지역변수가 없기때문에 this.생략가능.
	}
