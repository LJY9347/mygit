...
public class InheritanceMain03 {
	...
	public static void main(String[] args) {
		...
		// SubClass var = new SuperClass(); -> 컴파일 오류
        // HybridCar car = new Car(100,100);
		car3.drive(); // 다형성으로 선언되어 있더라도, override되어 있는 메서드가 호출됨.
        //car3.charge(100); //-> casting을 하지 않으면 SubClass(실제 객체의) 모든 메서드를 사용할 수는 없음.
		((HybridCar)car3).charge(100); //-> casting을 하면, SubClass의 모든 메서드를 사용할 수 있음.
		
		// 다형성의 장점: 코드의 재사용성(re-usability)을 높여줌.
		// (예) 배열 선언, 파라미터 선언 등에서 상위 타입으로 선언한 변수에 하위 타입 객체들을 할당할 수 있다.
		Car[] cars = new Car[3]; 
		cars[0] = car1; // new Car(...)
